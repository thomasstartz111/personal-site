      ...
      >
        <div className="min-h-screen flex flex-col">
          <header className="border-b border-zinc-300">
            <nav className="max-w-5xl mx-auto px-4 sm:px-6 py-4 sm:py-5 flex items-center justify-between gap-4">
              ...
              <ul className="flex gap-3 sm:gap-6">
                {navLinks.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-zinc-500 text-xs sm:text-sm hover:text-zinc-900 transition-colors focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:ring-offset-2 rounded"
                    >
                      {link.label}
                    </Link>
                  </li>
